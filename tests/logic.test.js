...
function createMockDocument() {
  ...
  return {
    getElementById: (id) => {
      if (!elements[id]) elements[id] = { textContent: '', innerHTML: '', href: '', style: { display: '' } };
      return elements[id];
    },
    addEventListener: () => {},
    elements
...
// Local date retrieval across time zones
...
// Free time calculation with non-overlapping activities
const sampleDay = {
  activities: [
...
// Free time calculation with overlapping activities
const overlapDay = {
  activities: [
...
// Handling when no itinerary exists
...
// Mocked geolocation success
{
  ...
  global.TripLogic = {
    getItineraryForDate: () => ({ accommodation: { address: 'Test' }, activities: [] }),
    getFreeTimeBlocks: () => [],
